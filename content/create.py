...
fields = [  # (field, default, comment)
    ...
    ('date',      today,
     '記事日付。YYYY.mm.DD または YYYY.mm.DD HH.MM 形式'),
    ('modified',  None,               '更新日。None の場合は date と同じ'),
    ('title',     None,
     '記事タイトル。content 内に # 要素がある場合は指定しない。'),
    ('category',  'blog',             'blog, news または page'),
    ('tags',      None,
     'Bioinformatics など、サイト参照。複数の場合カンマ区切り'),
    ('author',    None,
     '記事執筆者の名前。姓またはHN、複数の場合カンマ区切り'),
    ('content',   None,
     '記事内容ファイル (.md/.ipynb) がすでにある場合はパスを指定'),
]
...
if __name__ == '__main__':
    ...
    for field, default, comment in fields:
        if shortused[field[0].lower()]:
            parser.add_argument('--{}'.format(field.lower()),
                                default=None, help=comment)
        else:
            parser.add_argument('--{}'.format(field.lower()),
                                '-{}'.format(field[0].lower()), default=None, help=comment)
            shortused[field[0].lower()] = True
    parser.add_argument('--filename', '-f', default=None,
                        help='--slug と同じ。両方指定すると slug が優先')
    parser.add_argument('--default', action='store_true',
                        default=None, help='未指定の項目を質問せずデフォルト値にする')
    ...
    for field, default, comment in fields:
        ...
        if args.default or getattr(args, field):
            print('{0}: {1}'.format(Field, getattr(
                args, field) or fields[field][1]))
            values[field] = getattr(args, field)
        else:
            print('{0} ({2}) [ {1} ]: '.format(
                Field, default, comment), end='')
            given = input().strip()
            if given:
                values[field] = given
            else:
    ...
    if values['category'].lower() in {'page', 'pages'}:
        ...
        directory = "./articles/{0}sy/{1}".format(
            schoolyear, values['category'])
        ...
        if os.path.isfile(contentfile):
            ...
            if suffix == 'md':
                ...
                if os.path.exists(filepath_ipynb):
                    ...
                    print("ファイル '{}' を作成 (コピー) しました。".format(
                        filepath_ipynb))
            else:
                print("ファイル '{}' は対応するファイル形式ではありません。指定を無視します。".format(
                    contentfile))
        else:
            print("ファイル '{}' は存在しません。指定を無視します。".format(
                contentfile))
    ...
    if suffix == '.md':
        try:
            ...
            print("画像保存用ディレクトリ'{}'を作成しました。".format(
                imagedirpath))
        except FileExistsError:
            if os.path.isdir(imagedirpath):
                print("画像保存用ディレクトリ'{}'はすでに存在します。".format(
                    imagedirpath))
            else:
                print("画像保存用ディレクトリ'{}'を作成できませんでした。".format(
                    imagedirpath))
