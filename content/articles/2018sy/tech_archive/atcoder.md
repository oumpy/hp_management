...
出力
硬貨を選ぶ方法の個数を出力せよ。
1. a, b, c, x = map(int, [input() for i in range(4)])
2. ans = 0
3. for i in range(a+1):
4.     for j in range(b+1):
5.         for k in range(c+1):
6.             if i * 500 + j * 100 + k * 50 == x:
7.                 ans += 1
8. print(ans)
```
...
1. n = int(input())
2. print(len(set(map(int, [input() for i in range(n)]))))
```
...
```
1. n = int(input())
2. a = sorted(list(map(int, [input() for i in range(n)])))
3. ans = 1
4. if n != 1:
...
1. s = input()
2. a=s.replace("eraser"," ")
3. b=a.replace("erase"," ")
4. c=b.replace("dreamer"," ")
5. d=c.replace("dream"," ")
6. if d.strip() == "":
7.     print("YES")
8. else:
...
別解答：(<http://delta114514.hatenablog.jp/entry/2018/03/15/014555> より引用)  
