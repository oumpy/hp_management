...
プレビューは 1 つがサイト 1 式分の容量を占めるので、溜まると GitHub Pages の容量上限にかかり本番サイトが更新されなくなります。
削除済みブランチのプレビューは自動更新・プレビュー作成のたびに自動で掃除されますが、
手動で整理したいときは GitHub の Actions タブから "Prune_Previews" ワークフローを実行してください
(孤児のみ / 指定ブランチ / 全部 / 一覧のみ、を選べます。残す個数の上限はレポジトリ変数 `PREVIEW_MAX` で設定できます)。
